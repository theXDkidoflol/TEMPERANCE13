...
/datum/status_effect/buff/arcyne_momentum
	...
	var/stacks = 0
	var/max_stacks = 10
	var/glow_colour = "#581616"
	var/crackle_colour = "#531010"
...
/datum/status_effect/buff/arcyne_momentum/proc/add_stacks(amount)
	...
	if(old_stacks < 3 && stacks >= 3)
		to_chat(owner, span_notice("The flow of vitae courses through my veins."))
		playsound(get_turf(owner), 'sound/magic/bloodcrack.ogg', 30, TRUE)
	if(old_stacks < 6 && stacks >= 6)
		to_chat(owner, span_warning("I am ready, let us end this juvenile dream."))
		playsound(get_turf(owner), 'sound/magic/bloodcrack.ogg', 50, TRUE)
	if(old_stacks < overcharge_threshold && stacks >= overcharge_threshold)
		to_chat(owner, span_boldwarning("I am about to burst from vitae. I must consume it."))
		playsound(get_turf(owner), 'sound/magic/bloodcrack.ogg', 70, TRUE)
...
/datum/status_effect/buff/arcyne_momentum/proc/enter_overcharge()
	...
	to_chat(owner, span_boldwarning("I am overloaded - strands of hardblood pierce through my body."))
